...
unsigned int nTargetSpacing = 1 * 20; // 20 seconds
unsigned int nStakeMinAge = 24 * 60 * 60; // 1 day min stake
unsigned int nStakeMaxAge = 3 * 24 * 60 * 60;// 3 days max stake
...
const string strMessageMagic = "Romancoin Signed Message:\n";
...
int64_t GetProofOfWorkReward(int nHeight, int64_t nFees, uint256 prevHash)
{
        ...
        int rand = generateMTRandom(seed, 6);
...
const int DAILY_BLOCKCOUNT =  4320;
...
int64_t GetProofOfStakeReward(int64_t nCoinAge, int64_t nFees, int nHeight)
{
	...
	if(nHeight < 30240)
	{
    int64_t nSubsidy = nCoinAge * 0.3 * 104 / 365 / COIN;   //first week - 10% per week(not accounting compounding and stuff)
	}
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Romancoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "The blue air is up there";
        CTransaction txNew;
        txNew.nTime = 1401126916;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0x4baf9ff69d60b2d0322ab3c9cb63717c9f964080e6c58548a06f7c8be6cae702"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xa0, 0xa0, 0xa0, 0xa3 };
